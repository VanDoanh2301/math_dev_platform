...
import com.dev.devmath.core.designsystem.theme.KptColorSchemeImpl
import com.dev.devmath.core.designsystem.theme.KptShapesImpl
import com.dev.devmath.core.designsystem.theme.KptTheme
import com.dev.devmath.core.designsystem.theme.KptTypographyImpl
...
/**
 * Extension functions and utilities for easier theme development
 */

/**
 * Checks if the current color scheme is a dark theme.
 * This is determined by checking if the background color is darker than the surface color,
 * or by checking if the background is generally dark.
 */
@Composable
fun KptColorScheme.isDark(): Boolean {
    // Simple heuristic: if background luminance is low, it's dark
    val backgroundLuminance = background.red * 0.299f + background.green * 0.587f + background.blue * 0.114f
    return backgroundLuminance < 0.5f
}

/**
 * Gets the appropriate text color for a given background color.
 * Returns onBackground for light backgrounds, onSurface for dark backgrounds.
 */
@Composable
fun KptColorScheme.textColorForBackground(backgroundColor: Color): Color {
    val luminance = backgroundColor.red * 0.299f + backgroundColor.green * 0.587f + backgroundColor.blue * 0.114f
    return if (luminance > 0.5f) onBackground else onSurface
}

/**
 * Gets the appropriate surface color variant based on elevation level.
 * Higher elevation levels return brighter surfaces in light theme and darker in dark theme.
 */
@Composable
fun KptColorScheme.surfaceForElevation(level: Int): Color {
    return when (level) {
